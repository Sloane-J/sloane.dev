            ...
            {images.map((_, idx) => (
              <button
                ...
                className={`h-0.5 rounded-none transition-all duration-300 ${
                  currentIndex === idx ? "w-6 bg-[#FF5733]" : "w-2 bg-white/30"
                }`}
...
// --- ProjectCard ---
const ProjectCard = memo(({ project, index, sectionProgress, totalProjects }: {
  ...
  const opacity =
    localProgress < 0 ? 0
    : localProgress < 0.2 ? localProgress * 5
    : localProgress > 0.8 ? (1 - localProgress) * 5
    : 1;

  const scale =
    localProgress < 0.2 ? 0.9 + localProgress * 0.5
    : localProgress > 0.8 ? 1 - (localProgress - 0.8) * 0.25
    : 1;
      ...
      <div className="w-full max-w-6xl mx-auto px-6 lg:px-10">
        ...
        <div className="grid lg:grid-cols-2 border border-[#1a1a1a] bg-[#080807]">
          ...
          <div className="flex flex-col gap-6 p-8 lg:p-10 border-b lg:border-b-0 lg:border-r border-[#1a1a1a]">
            ...
            >
              <p className="font-mono text-xs text-[#666] leading-[1.8]">
                {project.description}
              </p>
            </div>
            ...
            >
              {project.tags.map((tag) => (
                <span
                  key={tag}
                  className="font-mono text-[10px] tracking-wide text-[#555] border border-[#222] px-2.5 py-1 rounded-sm"
                >
                  {tag}
                </span>
              ))}
            ...
            >
              {project.liveUrl !== "#" && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 bg-[#FF5733] hover:opacity-85 transition-opacity text-white font-syne font-bold text-xs tracking-wide px-5 py-3 rounded-sm"
                >
